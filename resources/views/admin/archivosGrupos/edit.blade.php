...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.archivosGrupo.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.archivos-grupos.update", [$archivosGrupo->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('grupo') ? 'is-invalid' : '' }}" name="grupo_id" id="grupo_id" required>
                    @foreach($grupos as $id => $grupo)
                        <option value="{{ $id }}" {{ ($archivosGrupo->grupo ? $archivosGrupo->grupo->id : old('grupo_id')) == $id ? 'selected' : '' }}>{{ $grupo }}</option>
                    @endforeach
                </select>
                @if($errors->has('grupo'))
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('texto') ? 'is-invalid' : '' }}" name="texto" id="texto">{!! old('texto', $archivosGrupo->texto) !!}</textarea>
                @if($errors->has('texto'))
