    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.horarios.update", [$horario->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('grupo') ? 'is-invalid' : '' }}" name="grupo_id" id="grupo_id" required>
                    @foreach($grupos as $id => $grupo)
                        <option value="{{ $id }}" {{ ($horario->grupo ? $horario->grupo->id : old('grupo_id')) == $id ? 'selected' : '' }}>{{ $grupo }}</option>
                    @endforeach
                </select>
                @if($errors->has('grupo'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('asignatura') ? 'is-invalid' : '' }}" type="text" name="asignatura" id="asignatura" value="{{ old('asignatura', $horario->asignatura) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('color') ? 'is-invalid' : '' }}" type="text" name="color" id="color" value="{{ old('color', $horario->color) }}" required>
                @if($errors->has('color'))
