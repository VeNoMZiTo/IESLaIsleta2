...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.citaPrevium.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.cita-previa.update", [$citaPrevium->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('asignatura') ? 'is-invalid' : '' }}" name="asignatura_id" id="asignatura_id" required>
                    @foreach($asignaturas as $id => $asignatura)
                        <option value="{{ $id }}" {{ ($citaPrevium->asignatura ? $citaPrevium->asignatura->id : old('asignatura_id')) == $id ? 'selected' : '' }}>{{ $asignatura }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('curso') ? 'is-invalid' : '' }}" name="curso_id" id="curso_id" required>
                    @foreach($cursos as $id => $curso)
                        <option value="{{ $id }}" {{ ($citaPrevium->curso ? $citaPrevium->curso->id : old('curso_id')) == $id ? 'selected' : '' }}>{{ $curso }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('fecha') ? 'is-invalid' : '' }}" type="text" name="fecha" id="fecha" value="{{ old('fecha', $citaPrevium->fecha) }}" required>
                @if($errors->has('fecha'))
